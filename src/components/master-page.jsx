...
export const MasterPage = (props) => {
    ...
    const onSubmitForm = (e) => {
        ...
        if (isFormValid()) {
            setLoading(true);
            SmsService.sendSMS(`+${phoneNumber}`, smsContent).then((response) => {
                setLoading(false);
                if (response.data.code === 0) {
                    setApiResponse(<Notification type="success" icon="check-circle" message={response.data.message} content={JSON.stringify(response.data.result)} />)
                } else {
                    setApiResponse(<Notification type="danger" icon="x-circle" message={response.data.message} content={JSON.stringify(response.data.result)} />)
                }
            }, error => {
                setLoading(false);
                setApiResponse(<Notification type="danger" icon="x-circle" message={error.message} />)
            });
        } else {
                ...
                <form onSubmit={(e) => onSubmitForm(e)}>
                    <div className="form-group">
                        <input type="number"
                            ...
                            onChange={(e) => { setPhoneNumber(e.target.value); setApiResponse(null) }} />
                    </div>
                    <div className="form-group">
                        <textarea type="text"
                            required
                            ...
                            onChange={(e) => { setSmsContent(e.target.value); setApiResponse(null) }} />
                    </div>
                    ...
                    <div className="form-group">
                        {
                            loading ? <Loader /> :
                                <button type="submit" className="btn btn-success form-control submit-btn" ><i className="bi bi-check2-circle" /> Submit</button>
                        }
                    </div>
                </form>
                {
